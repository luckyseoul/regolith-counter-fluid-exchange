...
- COLD_REVIEW.md — Independent skeptical audit (2026-06-09) covering enablement risks (112), distributor dependency/narrative, seals/hardware blocker (user's own admissions), prior art differentiation, data history, 2-page leverage report assessment, and custom-gpu-dem spin-off. Added for the record.
...
The body-force term in the primary evidence is framed as the sub-grid model for the *physical* momentum transfer from gas jets of the *claimed* distributor design (per spec and FIG. 06). This is standard practice in DEM for FB with unresolved jets. The drag-fix verifies that the gas introduction hardware is essential for the effect at low reference P. No higher system P or heavy vessel/gasket/safety testing is required; the low-P operation is preserved with the distributor as the key feature providing injection momentum. In contrast to prior mechanical approaches (e.g., rotating auger counterflow recuperators), the design introduces no dynamic rotary seals or rotating components in the primary abrasive regolith path; such seals would require physical prototypes and extended wear/leakage testing for credible validation — costs and timelines typically unaffordable in resource-constrained programs. The iron-collision agitation + static distributor + physical-lid containment approach supports full enablement via reproducible modeling and DEM without that prototype burden.
