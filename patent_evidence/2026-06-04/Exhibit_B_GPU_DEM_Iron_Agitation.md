...
**RawKernel (high sustained GPU util)**: compute_forces_raw (single launch, inner j loop) is now default in highN migration + benchmark. Validated bit-exact to high-level reference via side-by-side on ckpt data (max |f| diff ~0.002, torque exact; float32). This addresses "not using the GPU much": kernels stay at 100% during the contact phase (one launch, no dozens of CuPy N² temporaries). (Cell-list path remains Python-loop limited for neighbor search; rewrite is next for >7k scale + full occupancy.)
