...
| Metric | Value | Source |
|--------|-------|--------|
| EMI (iron / no-iron bed height ratio) | **3.8657×** (400s) → **4.6977×** (500s) → **6.3805×** (700s) → **7.1036×** (800s) → **7.6516×** (900s) → **8.0445×** (1000s) → **8.53× peak (1300s)** → **7.89× (2000s, RawKernel)** | Direct from Rung1_HighN_Primary_Audit_6500.md/.json (np.load on raw ckpts + continuation to 2000); exact no-iron baseline reg bed 3.2307 mm @400s; with-iron reg 12.4889 → 27.5656 mm peak (1300s) then 25.48 mm (2000s, iron 23.92); see audit for full table + KE/dead/zmin + lid-pile dynamics |
...
| With-iron (high-N) | reg 12.4889±7.1141 mm (400s) → 20.6136±11.1155 (700s) → 25.9894±11.8965 mm (1000s) → **27.5656±11.45 mm peak (1300s, iron 27.25)** then 25.48±13.88 mm (2000s, iron 23.92); dead% 0 early → 11.05% (1000s) → 20.0% (1300s) → 28.8% (2000s, lid pile); vmean 52.33→37.44 m/s; KE bias 2138.9→631×; zmax 27.27→41.8 mm (lid cap, stable); 100% inside all ckpts | Direct np.load audit + continuation |
...
| Compute path | compute_forces_raw (single RawKernel launch) default in migration/benchmark/coarse for high sustained util (kernels ~100% during contacts; SURFACE=0 for Rung1 no-coh; unit tests match high-level dF~1e-9; highN high-level N^2 mem-unreliable so Raw authoritative) | dem_kernels.py + audit; Raw used for entire highN primary evidence |

**Lid + freeboard (physical scale enablement at high-N)**: Migration uses lid+freeboard (40 mm soft / 60 mm hard) + opt stepper from step 0. Bed building under physical cap then stabilizing/piling (see Rung1_HighN_Primary_Audit_6500.md for exact table + extension): EMI 3.8657× (400s) → 6.3805× (700s) → **8.0445× (1000s)** → **8.53× peak (1300s)** → 7.89× (2000s via RawKernel); reg mean z 12.4889 → 25.9894 → **27.5656 mm peak (1300s, iron 27.25)** then 25.48 mm (2000s, iron 23.92) toward 60 mm lid; 100% inside on all raw ckpts (original + continuation); zmax 41.3→41.8 mm (stable at cap). Early dead% 0.0% in with-iron vs 86.66% in no-iron control (vmean 0.403 m/s, zmax~10 mm); KE bias 1000–2500× (sustained hundreds× at long times). Mechanism (iron agitation / mobilization differential) robust at physical heights under lid constraint. Note rising dead% and slight mean recession at long times (lid-pile low-vel zones as mass accumulates near cap) — still 100% contained, differential vs control clear and strong (EMI ~7.9-8.5×). Continuation used continue_highn_rung1.py (standard 2.8 dist from settled state).

**RawKernel + Cell-list hotpath (high sustained GPU util, post-rewrite)**: compute_forces_cell_list (thin shim) / compute_forces_cell_raw (single RawKernel launch containing the 27-neighbor search) is now the default production path in highN migration/continue/benchmark/run_rung2_cell_list + optimized_stepper. Device-only build_cell_list (no .get() sync, no Python for-range sentinel fill). SURFACE zeroed for Rung1 no-reg-coh. Matches high-level exactly on unit tests (dF ~1e-9 on N=2); on N=6500 high-level N^2 is mem-unreliable, cell+Raw (low mem, kernels 100% fed in contact phase) is authoritative for primary evidence and future sensitivities. The previous Python `for c in range(grid) + neighbor loops` (slow path in old cell_list) have been removed from the hot path. Cell-list enables practical N>7k-20k+ and long runs.
