...
- Logs: /tmp/rung5_slice.log (contains exact "rung5 done. Final bed: 10404.50±5708.47 mm (zmax=22704mm zmin=0.49mm inside=100.0%) dead%=3.8" + every-500 "bed=... inside=100.0%" to 500k + proxy), /tmp/rung5_status.txt (100% RUN COMPLETE + CONTAINED ✅ + ).  
...
**Prepared for**: Utility filing support (drawings, spec, evidence for enablement).  
**Exhibit files** (this directory):
- `EXECUTIVE_SUMMARY.md` — one-page summary for the filing package
...
- `patent_drawings/FIG_01_system_overview.svg/pdf` (5-stage counter-current schematic)
- `patent_drawings/FIG_03_iron_agitation_rung5_final.svg/pdf` (rung5_step200000.npz)
- `patent_drawings/FIG_03_iron_agitation_rung5_500k_final.svg/pdf` (rung5_step500000.npz)
- `patent_drawings/FIG_05_effectiveness_vs_pressure.svg/pdf` (lumped curve, 0.14 bar / 75.6%)
- `patent_drawings/FIG_06_distributor_rung0_final.svg/pdf` (rung0_step500000.npz)
- `patent_drawings/FIG_07_rung5_mobilization_progression.svg/pdf` (all 334 contained Rung 5 ckpts; 200k/500k callouts)
- `patent_drawings/FIG_02_stage_cross_section.svg/pdf` (single-stage cross-section, reference numerals 100–120)
- `patent_drawings/FIG_04_countercurrent_transfer.svg/pdf` (five-stage counter-current schematic, Rung 4 ~230 particle transfer callout)

**Specification draft**: `patent_specification_draft.md`

**Next**: Internal review complete (Rung1 fixed + lid enablement updates included). .docx assembly complete (this session): RCFX_Patent_Evidence_Package_2026-06-04.docx (TOC + exec + matrix + cold review + Exhibits A–E + filing readiness + new audits) and RCFX_Specification_Support_Draft_2026-06-04.docx. Full figure set FIG. 1–7 complete for filing support.

**Filing checklist**: `FILING_READINESS.md` (containment audit 334/334 on Rung 5 + Rung1 99k 100%, new lid demo, package paths).

**Cold review (math validation + independent claim-by-claim)**: `COLD_CLAIMS_AND_MATH_REVIEW.md` (added 2026-06-04 session). Covers: exact reproduction of 75.6%/221 W/robustness; power formula bug + U_G misalignment in lumped model; Rung1 EMI 107.9× / "100.0% inside" factual failure on raw .npz (Rung0/5 clean); unphysical velocities/loft in iron DEM runs; full 31-claim support matrix vs. Rev 5.2 PDF + artifacts. Recommendations for hygiene patches before filing use.

*Only verified raw post-containment contained .npz numbers used. Rung 0 and Rung 5 locked.* 
