...
| Counter-current solids transfer | Staging § | FIG. 4 (planned) | Exhibit C (Rung 4 ~230 particles) | Partial — schematic FIG. 4 pending |
...
1. **FIG. 2** (single-stage cross-section) — referenced in spec, not yet drawn.
2. **FIG. 4** (counter-current transfer) — Rung 4 data exists; formal schematic not generated.
3. **Formal claims text** — matrix maps to functional elements; independent/dependent claim wording not in repo.
