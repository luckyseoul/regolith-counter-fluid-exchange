...
**2026-06-05**: New provisional patent application generated in /home/nick/rcfx/patent_provisional/2026-06-05/ incorporating *all* campaign learnings (high-N Rung1 physical lid N=6500 + 10k scale via reproducible addition, cell-list, iron size leverage on dead/KE to 45k×, full sensitivities + campaign at evolved state, runner as enablement tool, S3/S4 drawings, 100% containment rules). Includes updated full spec, bundle index, cover info, suggested claims for conversion, and symlinked drawings. References the latest evidence package, COLD, and raw ckpts/reports. Ready for counsel to assemble into formal provisional (MD source + drawings). Plan "Remaining" now reflects post-provisional polish only.
