...
- What comes after: Cell-list integration for high-N evidence code (to scale beyond brute O(N^2) for larger N or faster long runs at sustained high VRAM/occupancy). Start by updating migration script and runners to prefer cell_list.compute_forces_cell_list. Then re-run high-N with cell for more steps/sensitivity. Also, more high-N sensitivity if needed, final package polish.
