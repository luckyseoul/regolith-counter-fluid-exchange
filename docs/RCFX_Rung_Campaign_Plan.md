...
- What comes after (in progress): Cell-list hotpath rewrite (remove Python `for c in range` + neighbor loops; current ext was ~0.15 steps/s, too slow; enables >7k + higher sustained util). Graph capture for full step (minimize host gaps). High-N sensitivities (iron size, U_G, fines %). Further extension if needed (>2000 steps). Final package polish + claims re-sweep vs latest highN numbers (now includes 2000s lid data). 
