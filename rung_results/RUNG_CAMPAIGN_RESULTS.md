...
**GPU DEM (Rung 5) Status (live)**: Re-launched/extended to 500k (resuming from 200k). Run completed clean (no active long-running pid, GPU 0% idle). Status bar: 100.00% (500000/500k) remain 0, ~1653 steps/min. Latest printed/ckpt rung5_step500000: bed=10404.50±5708.47 mm (zmax=22704mm zmin=0.49mm inside=100.0%) dead%=3.8 ; CONTAINED ✅ (np.load verified). Log: final "rung5 done. Final bed: 10404.50±5708.47 mm (zmax=22704mm zmin=0.49mm inside=100.0%) dead%=3.8" + "Rung5 proxy (iron vs reg bed for mobilization): iron_bed=12584.1mm reg_bed=9781.8mm". 334 ckpts (latest 500000). Monitor ended on done. Full ps/nvidia + direct np.load (inside=100.0% mask x/y[0,0.016] z>=0 + zmin>=0 + CONTAINED) + log cross-check + status bar before lock. All post-containment contained raw .npz only.

**Infrastructure (cell-list hotpath rewrite)**: Post highN work, the cell-list path was completed: build_cell_list is now fully device-side (no .get() + Python for sentinel fill); compute_forces_cell_list is a thin shim to the single-launch RawKernel cell implementation (neighbor search inside CUDA, no Python `for c in range` + per-cell loops in hot path). HighN runners default to brute Raw for citable compatibility; --use-cell-list (migrate) or direct import available for scale (N>7k sensitivities, 20k+ particles). See updated plan, Exhibit B, COLD, FILING_READINESS. "Rung 5 locked... now invoking patent skills" + cell-list now production-capable. Rung 0 remains fully locked (500k).
