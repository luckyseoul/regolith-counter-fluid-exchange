...
Usage:
  ...
  watch -n 5 python rung0_status.py     # auto-refresh every 5s
  cat /tmp/rung0_status.txt             # even faster one-shot check

Also writes a fresh /tmp/rung0_status.txt every run.
...
CKPT_DIR = BASE / "rung0_checkpoints"
LOG_PATH = Path("/tmp/rung0_slice.log")
...
def get_latest_ckpt():
    files = list(CKPT_DIR.glob("rung0_step*.npz"))
    if not files:
        return None, 0
    def sn(p):
...
def get_process_and_gpu():
    ...
    try:
        ...
        for line in out.splitlines():
            if "rung0" in line and "python" in line and "grep" not in line.lower():
...
def main():
    ...
    ckpt_files = list(CKPT_DIR.glob("rung0_step*.npz"))
    ...
    # Build output
    lines = []
    lines.append("Rung 0 Distributor Backfill — Status Bar")
    lines.append("=" * 72)
    ...
    lines.append(f"Log: {LOG_PATH}   (monitor active)")
    lines.append("")

    if log_info["is_done"]:
        lines.append("*** RUN COMPLETE — check final 'rung0 done' line ***")
    else:
        lines.append("Only 100.0% inside + zmin>=0 numbers are citable for patent evidence.")
        lines.append("Run this often:  python rung0_status.py   |   watch -n 5 python rung0_status.py")
        lines.append("Fast check:  cat /tmp/rung0_status.txt")
    ...
    try:
        with open("/tmp/rung0_status.txt", "w") as f:
            f.write(output + "\n")
